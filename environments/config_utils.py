...
def gen_eval_gamut(eval_cfgs):
    ...
    for cur_eval_cfg in splat_multiplex(omegaconf.OmegaConf.create(eval_cfgs)):
        ...
        IS_LOWER_RANGE_ENV = ("widow" in eval_cfgs["env_key"][0]) or ("hopper" in eval_cfgs["env_key"][0])
        print("IS LOWER RANGE")
        print(IS_LOWER_RANGE_ENV)

        SET_HIGH = 2.0 if IS_LOWER_RANGE_ENV else 10.0
