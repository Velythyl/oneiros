...
class FrameStack(Wrapper):
    def __init__(self, env, num_frames, device):
        super().__init__(env)
        ...
        self.stacks = torch.zeros(self.stacks_shape, dtype=torch.float32, requires_grad=False, device=device)
        ...
        # used with gather
        roll_indices = torch.arange(self.num_frames+1, device=device, requires_grad=False, dtype=torch.int64)
        roll_indices[-1] = 0
        roll_indices = roll_indices[1:]
        self.roll_indices = roll_indices[None,:,None].expand(self.num_envs, self.num_frames, obs_size)
        self.env_indices = torch.arange(self.env.num_envs, requires_grad=False, device=device, dtype=torch.int64)
