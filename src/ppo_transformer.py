...
from graph_utils.get_transformer import get_transformer
...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        node_dim = envs.single_observation_space.shape[-1]

        self.actor_and_critic_and_logstd = get_transformer(node_dim=node_dim, depth=6)
...
class PPO:
    def __init__(self, train_envs, all_hooks, learning_rate, num_steps, total_timesteps, num_minibatches, anneal_lr, gamma, gae_lambda, update_epochs, clip_coef, norm_adv, clip_vloss, ent_coef, vf_coef, max_grad_norm, target_kl, **kwargs):
